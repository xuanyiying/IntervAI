...
import { MessageRole, type MessageItem, type Job, type AttachmentStatus, type InterviewQuestion } from '../../../types';
...
export const useChatItems = ({ messages, localItems, streamingContent, isStreaming }: UseChatItemsProps) => {
    ...
    if (messages.length > 0) {
      mappedItems = messages.map((msg) => {
        ...
        return {
          ...
          optimizedMarkdown: msg.metadata?.optimizedMarkdown as string | undefined,
          suggestions: msg.metadata?.suggestions as MessageItem['suggestions'] | undefined,
          interviewQuestions: msg.metadata?.interviewQuestions as InterviewQuestion[] | undefined,
          attachmentStatus: msg.metadata?.attachmentStatus as AttachmentStatus | undefined,
        };
      });
    }
    ...
    if (isStreaming && streamingContent) {
      ...
      if (!isAlreadyPersisted) {
        ...
        const sections = [
          ...
          { key: '项目经验', tip: '✅ 已优化项目经验，突出了个人在项目中的核心贡献' },
