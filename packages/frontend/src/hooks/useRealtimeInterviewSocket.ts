...
export interface RealtimeInterviewSocketOptions {
...
export interface RealtimeInterviewSocketReturn {
  isConnected: boolean;
...
export const useRealtimeInterviewSocket = (
  options?: RealtimeInterviewSocketOptions
): RealtimeInterviewSocketReturn => {
  ...
  const setupSocketEvents = useCallback(
    (socket: Socket) => {
      ...
      socket.on('joined_session', (data) => {
        console.log('Joined realtime interview session:', data.sessionId);
      });
  ...
  useEffect(() => {
    ...
    socketRef.current = io(`${serverUrl}/realtime-interview`, {
