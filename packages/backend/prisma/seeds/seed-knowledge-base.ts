...
export async function seedKnowledgeBase(prismaClient: PrismaClient) {
  console.log('🌱 Seeding knowledge base to PostgreSQL...\n');

  const apiKey = process.env.QWEN_API_KEY || process.env.OPENAI_API_KEY;
  if (!apiKey) {
    console.warn(
      '⚠️  No embedding API key found. Skipping knowledge base seeding.'
    );
  ...
  if (useOpenAI) {
    ...
    console.log('Using Qwen/DashScope for embeddings...');
    embedder = new QwenEmbeddings({
      apiKey: apiKey,
  ...
  for (const item of allItems) {
    try {
      ...
      const existing = await prismaClient.knowledgeBase.findFirst({
        where: {
          source: item.source,
      ...
      try {
        const embeddings = await embedder.generate([item.content]);
        embedding = embeddings[0];
      } catch (embedError) {
        console.warn(
          `⚠️  Embedding failed for item, storing without embedding: ${item.content.substring(0, 50)}...`
        );
        embedding = [];
      }
