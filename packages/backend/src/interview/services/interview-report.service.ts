...
请确保返回纯JSON格式，不要包含任何其他文字。`;
  ...
  private validateAndNormalizeAnalysis(
    ...
    return {
      ...
      dimensions,
      strengths: Array.isArray(parsed.strengths) ? parsed.strengths.slice(0, 5) : ['表现稳定', '态度认真'],
      improvements: Array.isArray(parsed.improvements) ? parsed.improvements.slice(0, 5) : ['需要更多练习', '可以更深入'],
      detailedAnalysis,
      recommendations: Array.isArray(parsed.recommendations) ? parsed.recommendations.slice(0, 5) : ['继续加强专业知识学习'],
      nextSteps: Array.isArray(parsed.nextSteps) ? parsed.nextSteps.slice(0, 5) : ['建议进行更多模拟面试'],
    };
  }
