...
/**
 * SiliconCloud API Response structure (Compatible with OpenAI)
 */
interface SiliconCloudAPIResponse {
  id: string;
  object: string;
  created: number;
  model: string;
...
/**
 * SiliconCloud Stream Response structure (Compatible with OpenAI)
 */
interface SiliconCloudStreamResponse {
  id: string;
  object: string;
  created: number;
  model: string;
...
/**
 * SiliconCloud Models List Response
 */
 ...
 */
export class SiliconCloudProvider implements AIProvider {
  readonly name = 'siliconcloud';
  private readonly logger = new Logger(SiliconCloudProvider.name);
  private readonly httpClient: AxiosInstance;
  private readonly config: SiliconCloudConfig;
  ...
  constructor(config: SiliconCloudConfig) {
    ...
    this.logger.log('SiliconCloud provider initialized');
  }

  /**
   * Call SiliconCloud API
   */
  async call(request: AIRequest): Promise<AIResponse> {
    return this.retryHandler.executeWithRetry(async () => {
      try {
        ...
        this.logger.debug(
          `Calling SiliconCloud API with model: ${request.model}`
        );

        const response = await this.httpClient.post<SiliconCloudAPIResponse>(
          '/chat/completions',
          payload
        );
  ...
  /**
   * Stream SiliconCloud API response
   */
  async *stream(request: AIRequest): AsyncIterable<AIStreamChunk> {
    try {
      const payload = { ...this.buildRequestPayload(request), stream: true };
      this.logger.debug(
        `Streaming SiliconCloud API with model: ${request.model}`
      );
        ...
        for (const line of lines) {
          ...
          if (trimmedLine.startsWith('data: ')) {
            try {
              ...
              const data: SiliconCloudStreamResponse = JSON.parse(jsonStr);
              const choice = data.choices[0];
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      this.logger.warn(
        `SiliconCloud provider health check failed: ${errorMessage}. Provider will be unavailable.`
      );
      return false;
    }
  }
  ...
  async listModels(): Promise<string[]> {
    try {
      const response =
        await this.httpClient.get<SiliconCloudModelsResponse>('/models');
      return response.data.data.map((model) => model.id);
    } catch (error) {
      this.logger.error(
        `Failed to list SiliconCloud models: ${error instanceof Error ? error.message : String(error)}`
      );
      return [];
    }
  }
