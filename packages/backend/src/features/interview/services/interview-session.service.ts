...
export class InterviewSessionService {
  ...
  private async triggerAsyncEvaluation(
    ...
    try {
      if (!session.optimization) {
        this.logger.debug(
          `Skipping evaluation for session ${sessionId}: no optimization context`
        );
        return;
      }
  ...
  private async generateFeedback(session: any) {
    if (!session.optimization) {
      this.logger.warn(
        `Skipping feedback generation for session ${session.id}: no optimization context`
      );
      return;
    }
    ...
    const resumeData = session.optimization.resume?.parsedData as unknown as
      | ParsedResumeData
      | undefined;
    const jobData = session.optimization.job?.parsedRequirements as unknown as
      | ParsedJobData
      | undefined;
