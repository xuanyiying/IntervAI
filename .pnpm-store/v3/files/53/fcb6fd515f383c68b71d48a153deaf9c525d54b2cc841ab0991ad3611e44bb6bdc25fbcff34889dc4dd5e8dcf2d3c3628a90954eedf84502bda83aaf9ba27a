...
const API_ENDPOINTS = { SEARCH: "/rest/api/3/search/jql" };
/**
...
var JiraProjectLoader = class extends BaseDocumentLoader {
	...
	documentConverter;
	personalAccessToken;
	constructor({ host, projectKey, username, accessToken, limitPerRequest = 100, createdAfter, personalAccessToken, filterFn }) {
		super();
		this.host = host;
		this.projectKey = projectKey;
	...
	async *fetchIssues() {
		...
		while (true) try {
			...
			const allIssues = [];
			for (const issue of data.issues) allIssues.push(issue);
...
//#endregion
export { JiraDocumentConverter, JiraProjectLoader, adfToText, jira_exports };
